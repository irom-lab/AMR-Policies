...
@gin.configurable
class RNNEncodingNetwork(network.Network):
  """Recurrent network."""
  ...
  def __init__(
    ...
    if len(rnn_size) == 1:
      cell = tf.keras.layers.StackedRNNCells([tf.keras.layers.SimpleRNNCell(rnn_size[0], dtype=dtype)])
        ...
        cell = tf.keras.layers.StackedRNNCells([
          tf.keras.layers.SimpleRNNCell(  # pylint: disable=g-complex-comprehension
              size,
              dtype=dtype)
    ...
    super(RNNEncodingNetwork, self).__init__(
