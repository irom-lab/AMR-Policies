...
python tf_agents/agents/sac/examples/v1/train_eval_rnn.py \
...
flags.DEFINE_integer('initial_collect_episodes', 1,
                     'Number of episodes to collect at the beginning of training using random policy')
flags.DEFINE_integer('collect_episodes_per_iteration', 1,
                     'Number of episodes to collect and be added to the replay buffer after every training iteration')
...
flags.DEFINE_integer('train_sequence_length', 20,
                     'Memory length for RNN')
...
def train_eval(
    ...
    """A simple train and eval for RNN SAC."""
    ...
    with tf.compat.v2.summary.record_if(
        ...
        initial_collect_op = dynamic_episode_driver.DynamicEpisodeDriver(
            ...
            num_episodes=initial_collect_episodes * num_parallel_environments).run()
        ...
        with sess.as_default():
            ...
            env_steps_op = env_steps.result()
            env_steps_op_call = sess.make_callable(env_steps_op)
            ...
            for _ in range(num_iterations):
                start_env_steps = env_steps_op_call()
                start_time = time.time()
                collect_call()
                env_time_acc = time.time() - start_time
                episode_steps = env_steps_op_call() - start_env_steps
                print('env steps collected:', episode_steps)
...
def main(_):
    ...
    print('actor_output_fc_layers', actor_output_fc_layers)
    print('actor_lstm_size', actor_lstm_size)
    ...
    print('critic_output_fc_layers', critic_output_fc_layers)
    print('critic_lstm_size', critic_lstm_size)
    ...
    train_eval(
        ...
        actor_output_fc_layers=actor_output_fc_layers,
        actor_lstm_size=actor_lstm_size,
        ...
        critic_output_fc_layers=critic_output_fc_layers,
        critic_lstm_size=critic_lstm_size,
        initial_collect_episodes=FLAGS.initial_collect_episodes,
