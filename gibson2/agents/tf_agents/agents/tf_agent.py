...
class TFAgent(tf.Module):
  ...
  def train(self, experience, weights=None, lam=0):
    """Trains the agent.
    Args:
    ...
    if self._enable_functions:
      loss_info = self._train_fn(experience=experience, weights=weights, lam=lam)
    else:
      loss_info = self._train(experience=experience, weights=weights, lam=lam)
  ...
  @abc.abstractmethod
  def _train(self, experience, weights, lam):
